...
export default function ApplicationDetail({ params }) {
  ...
  async function fetchApp() {
    ...
    if (data?.fit_score != null) {
      setFit({ fit_score: data.fit_score, ...data.fit_details });
    }
  }

  function saveResume(text) { setResumeText(text); localStorage.setItem("resumeText", text); }
  ...
  async function computeFit() {
    ...
    try {
      ...
      if (data.error) throw new Error(data.error);
      setFit(data);
      await supabase.from("applications").update({ fit_score: data.fit_score, fit_details: { matching_strengths: data.matching_strengths, missing_or_weak: data.missing_or_weak, summary: data.summary } }).eq("id", id);
    } catch (e) { setError(e.message); }
  ...
  if (!app) return <div className="flex items-center justify-center h-40"><div className="w-5 h-5 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" /></div>;
    ...
    <div className="fade-up max-w-4xl">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <h1 className="text-2xl font-bold text-[#0F1724]">{app.role_title}</h1>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-5 mb-5">
        <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
          Your Resume <span className="text-slate-300 normal-case font-normal">(saved in browser only — required for AI features)</span>
        </label>
        <textarea value={resumeText} onChange={e => saveResume(e.target.value)} rows={5}
          ...
          placeholder="Paste your resume as plain text here. It's stored only in your browser and sent directly to the AI when you generate." />
      </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          ...
          {fit ? (
            <div className="space-y-4">
              ...
              <div>
                ...
                <ul className="space-y-1">
                  {fit.matching_strengths?.map(s => <li key={s} className="text-xs text-slate-600 flex gap-1.5"><span className="text-green-400 mt-0.5">•</span>{s}</li>)}
                </ul>
              </div>
              <div>
                <p className="text-xs font-semibold text-red-500 mb-1.5">✗ Gaps to address</p>
                <ul className="space-y-1">
                  {fit.missing_or_weak?.map(s => <li key={s} className="text-xs text-slate-600 flex gap-1.5"><span className="text-red-400 mt-0.5">•</span>{s}</li>)}
                </ul>
              </div>
            </div>
          ) : (
            <div className="text-center py-8 text-slate-300">
              ...
              <p className="text-sm">Add your resume and score this role</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-5">
          ...
          {letter ? (
            <textarea value={letter} onChange={e => setLetter(e.target.value)} rows={14}
              className="w-full border border-slate-100 rounded-xl px-3 py-2.5 text-sm text-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none bg-slate-50" />
          ) : (
            <div className="text-center py-8 text-slate-300">
              <div className="text-4xl mb-2">✉</div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {app.analysis?.required_skills?.map(s => <span key={s} className="bg-blue-50 text-blue-600 text-xs px-2.5 py-1 rounded-full border border-blue-100">{s}</span>)}
              </div>
            </div>
            <div>
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Nice to have</p>
              <div className="flex flex-wrap gap-1.5">
                {app.analysis?.nice_to_have_skills?.map(s => <span key={s} className="bg-slate-100 text-slate-500 text-xs px-2.5 py-1 rounded-full">{s}</span>)}
              </div>
            </div>
          </div>
        </div>
      ...
      <details className="mt-4 text-sm text-slate-400 cursor-pointer">
        <summary className="hover:text-slate-600">View original job description</summary>
